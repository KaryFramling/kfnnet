...
explain(iris_test, explanation, n_labels = 3, n_features = 4)
...
ciu <- ciu.new(model, abs.min.max=matrix(c(0,1,0,1,0,1), ncol = 2, byrow = T), output.names=out.names)
CI.CU <- ciu$explain(iris_test, ind.inputs.to.explain=c(1), in.min.max.limits=c.minmax)
CI.CU
inp.ind.to.plot <- 3
out.ind.to.plot <- 2
ciu$plot.CI.CU(iris_test, ind.input=c(inp.ind.to.plot), ind.output=c(out.ind.to.plot), in.min=in.mins[inp.ind.to.plot], 
           in.max=in.maxs[inp.ind.to.plot], n.points=40, xlab=colnames(iris)[inp.ind.to.plot], ylab=out.names[out.ind.to.plot])
ciu$plot.CI.CU.3D(iris_test, ind.inputs=c(3,4), ind.output=3, in.mins=in.mins, in.maxs=in.maxs, n.points=40)
